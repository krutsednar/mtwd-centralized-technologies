...
<html>
<head>
    <title>DTR — {{ strtoupper($division->name) }} — {{ \Carbon\Carbon::createFromDate($year, $month, 1)->format('F Y') }}</title>
    <style>
        @page {
            size: 8.5in 13in;
            margin: 0.3in;
...
<div class="print-bar no-print">
    <form method="GET" action="{{ route('dtr.print.division', $division->id) }}" style="display:flex;align-items:center;gap:12px;width:100%">

        <label>Division:
            <select name="division_id" onchange="this.form.action='{{ url('/dtr/print/division') }}/'+this.value">
        ...
        <button type="button" onclick="window.print()" style="background:#16a34a;">
            🖨 Print All ({{ $profiles->count() }} employees)
        </button>

        <a href="/hris/attendances" class="end">
            ⬅ Back
        </a>
...
@empty
    <div style="text-align:center; padding: 40px; font-size: 14px; color: #666;">
        No employees found in {{ $division->name }} for {{ \Carbon\Carbon::createFromDate($year, $month, 1)->format('F Y') }}.
    </div>
@endforelse
